...
In this section, we demonstrate with an example that there exist sporadic  segmented self-suspending task sets that both \textbf{(i)} are schedulable \emph{without} period enforcement and \textbf{(ii)} are not schedulable under the period enforcer.
...
\textbf{XXX TODO:} It would be nice to show a schedule for the uncontrolled example.\textbf{XXX}

\textbf{XXX TODO:} We should review how the period enforcer works here, so that the reader may understand the schedule that demonstrates the miss.\textbf{XXX}
...
\section{How to Derive the Corresponding Deferrable Task Set}
\label{sec:convert}
